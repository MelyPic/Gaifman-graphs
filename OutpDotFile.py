...
from subprocess import check_call #from .dot to .png


ClanList = []
SingletonNodes = []

DictColors=['white','black','blue','blueviolet','brown','burlywood','cadetblue','chartreuse','coral','crimson','cyan','darkmagenta','darkorange','deeppink','deepskyblue','forestgreen','gold','greenyellow','hotpink','orangered','pink','red','seagreen','yellow']
		...
		while actnode < len(Clan.nodes)-1:
			...
			while restnode < len(Clan.nodes):
				...
				if MyGraph[int(Indx[0])][int(Indx[1])][0]== '0':
					...
					colr = int(MyGraph[int(Indx[0])][int(Indx[1])][0])%23
					print('clase: ', MyGraph[int(Indx[0])][int(Indx[1])][0])
